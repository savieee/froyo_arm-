...
def _is_video_capture_device(path: str) -> bool:
    """Checks via the V4L2 API whether a /dev/video* node supports video capture.

    Filters out nodes that cv2.VideoCapture cannot use: UVC metadata nodes, and
    output-only devices such as an idle v4l2loopback (opening those can hang OpenCV).
    Returns True on non-Linux errors/platforms so scanning falls back to trying the device.
    """
    import fcntl
    import struct
    ...
    try:
        with open(path, "rb", buffering=0) as fd:
            caps_struct = bytearray(104)  # sizeof(struct v4l2_capability)
            fcntl.ioctl(fd, VIDIOC_QUERYCAP, caps_struct)
            capabilities, device_caps = struct.unpack_from("<II", caps_struct, 84)
            effective_caps = device_caps if capabilities & V4L2_CAP_DEVICE_CAPS else capabilities
            return bool(effective_caps & V4L2_CAP_VIDEO_CAPTURE)
    except OSError:
        logger.debug(f"Could not query V4L2 capabilities for {path}, skipping it.")
        return False
...
class OpenCVCamera(Camera):
    ...
    def find_cameras() -> list[dict[str, Any]]:
        ...
        if platform.system() == "Linux":
            ...
            targets_to_scan = [str(p) for p in possible_paths if _is_video_capture_device(str(p))]
        else:
